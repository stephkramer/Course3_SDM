...
```
What do you actually see? Which method was used? Check the help! (?pairs). To make it more explicit, set the argument inside the function.

```{r}
# Plot the differences (rsiduals) between the rasters:
diff <- raster::corLocal(x = agg_pred[[1]], y = agg_pred[[3]], method ='spearman')
```
